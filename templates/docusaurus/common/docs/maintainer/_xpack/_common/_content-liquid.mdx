...
- [`@types/node`](https://www.npmjs.com/package/@types/node?activeTab=versions)
  locked to the latest version that still matches the oldest supported node ({{packageEnginesNodeVersionMajor}}).
...
npm init --yes
npm install {{packageScopedName}}@test
```
...
```bash
npm install --location=global {{packageScopedName}}@test
```
