...
{%- if topConfig.useSelfHostedRunners %}
...
{%- if platform == 'linux-arm64' %}{% assign names_array = names_array | concat: 'ampere' %}{% assign single_runner = true %}{% endif %}
{%- endfor %}
...
For {% if dual_runners_array.size > 1 %}`{{dual_runners_array | join: "` & `"}}`, which have{% else %}`{{dual_runners_array}}`, which has{% endif %} additional memory, and can run multiple jobs in parallel, start two runners:
...
{%- if topConfig.useSelfHostedRunners != true and isMacOS %}
...
{%- if topConfig.permalinkName == 'wine' %}
...
npm --prefix ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets install && \
xpm install -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets && \
xpm run test-release -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets
```
