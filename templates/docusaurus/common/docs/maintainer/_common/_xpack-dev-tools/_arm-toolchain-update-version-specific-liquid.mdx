...
- Check out the upstream branch mentioned in the release notes
  (such as <code>users/ARM/binutils-{{websiteConfig.binutilsVersionMajor}}_{{websiteConfig.binutilsVersionMinor}}-branch</code>)
...
- Add a tag such as <code>binutils-{{websiteConfig.binutilsVersionMajor}}.{{websiteConfig.binutilsVersionMinor}}-{{websiteConfig.triplet}}-{versionMajor()}.{versionMinor()}.rel{{websiteConfig.armSubRelease}}</code>
...
- Check out the branch mentioned in the release notes
  (such as <code>gdb-{{websiteConfig.gdbVersionMajor}}-branch</code>)
- **Push** it to origin with the same name
- Enable a filter on the current branch
- Identify the commit ID from release notes or manifest.txt (search back in time)
- Add a tag such as <code>gdb-{{websiteConfig.gdbVersionMajor}}-{{websiteConfig.triplet}}-{versionMajor()}.{versionMinor()}.rel{{websiteConfig.armSubRelease}}</code>
- **Push** the tag to origin
- Check the tag at https://github.com/xpack-dev-tools/binutils-gdb/tags/
...
- Enable a filter on the current branch
- Identify the commit ID from release notes or manifest.txt (search back in time)
- Create a branch like <code>arm-{versionMajor()}-{{websiteConfig.triplet}}-{versionMajor()}.{versionMinor()}.rel{{websiteConfig.armSubRelease}}-xpack</code>
- Create a tag like <code>arm-{versionMajor()}-{{websiteConfig.triplet}}-{versionMajor()}.{versionMinor()}.rel{{websiteConfig.armSubRelease}}</code>
- **Push** the tag to origin
...
- Enable a filter on the current branch
- Identify the commit ID from release notes or manifest.txt (search back in time)
- Create a tag like <code>newlib-{{websiteConfig.newlibVersion}}-{{websiteConfig.triplet}}-{versionMajor()}.{versionMinor()}.rel{{websiteConfig.armSubRelease}}</code>
- **Push** the tag to origin
...
the <code>{versionMajor()}.{versionMinor()}.rel{{websiteConfig.armSubRelease}}-darwin-x86_64-{{websiteConfig.triplet}}-manifest.txt</code> file
