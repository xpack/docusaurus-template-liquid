...
title: Maintainer's Guide{% if packageWebsiteConfig.customFields.usePluralGuides == "true" %}s{% endif %}
...
keywords:
  ...
  - build{% if packageWebsiteConfig.customFields.hasCustomMaintainer != "true" %}
  - releases
  - publish
  - npmjs{% endif %}
...
{% if packageWebsiteConfig.customFields.hasCustomMaintainer == "true" %}

import CustomMaintainer from './_custom-maintainer.mdx'

{/* ------------------------------------------------------------------------ */}

# Maintainer's Guide{% if packageWebsiteConfig.customFields.usePluralGuides == "true" %}s{% endif %}
...
# How to make new xPack {{packageConfig.longName}} releases
...
[![GitHub issues](https://img.shields.io/github/issues/xpack-dev-tools/{{gitHubProjectName}}.svg)](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/issues/)
[![GitHub pulls](https://img.shields.io/github/issues-pr/xpack-dev-tools/{{gitHubProjectName}}.svg)](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/pulls)

This page is designed for maintainers of the
**xPack {{packageConfig.longName}}**
...
- https://github.com/xpack-dev-tools/{{gitHubProjectName}}.git

To clone the **stable** branch (`xpack`), run the following commands in a
terminal (on Windows use the _Git Bash_ console):

```sh
rm -rf ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git && \
git clone https://github.com/xpack-dev-tools/{{gitHubProjectName}}.git \
  ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git
```
...
```sh
git -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git pull
```
...
In the `xpack-dev-tools/{{gitHubProjectName}}` Git repo:
...
- https://github.com/xpack-dev-tools/{{gitHubProjectName}}/issues
- https://github.com/xpack-dev-tools/{{gitHubProjectName}}/pulls
...
```sh
rm -f ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets/package-lock.json
git -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git pull

xpm run install -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets

git -C ~/Work/xpack-dev-tools/xbb-helper-xpack.git pull
xpm link -C ~/Work/xpack-dev-tools/xbb-helper-xpack.git

xpm run link-deps -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets

xpm run deep-clean --config darwin-x64  -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets && \
xpm install --config darwin-x64 -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets && \
xpm run build-development --config darwin-x64 -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```

For a debug build:

```sh
xpm run build-development-debug --config darwin-x64 -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
{% for platform in platforms_array -%}
{% if platform == "darwin-x64" %}{% assign names_array = names_array | concat: "xbbmi" %}{% assign single_runner = true %}{% endif -%}
{% if platform == "darwin-arm64" %}{% assign names_array = names_array | concat: "xbbma" %}{% assign single_runner = true %}{% endif -%}
...
{% if platform == "linux-arm64" %}{% assign names_array = names_array | concat: "xbbla" %}{% assign dual_runner = true %}{% assign dual_runners_array = dual_runners_array | concat: "xbbla" %}{% endif -%}
...
```sh
xpm run check-space -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
```sh
xpm run clear-all-projects-builds -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
```sh
xpm run generate-workflows -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
```sh
{% for name in names_array -%}
xpm run trigger-workflow-build-{{name}} -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
{% endfor -%}
{% comment %}{% endcomment %}```
...
Settings → Action →
[Secrets](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/settings/secrets/actions)
page.
...
[GitHub Actions](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/actions/) page.
...
{% if isLinuxX64 %}<li><b>trigger-workflow-test-docker-linux-x64</b></li>{% endif %}
{% if isLinuxArm %}<li><b>trigger-workflow-test-docker-linux-arm</b></li>{% endif %}
</ul>

```sh
xpm run trigger-workflow-test-prime -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
{% if isLinuxX64 %}xpm run trigger-workflow-test-docker-linux-x64 -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets{% endif %}
{% if isLinuxArm %}xpm run trigger-workflow-test-docker-linux-arm -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets{% endif %}
```
...
```sh
xpm run trigger-travis-macos -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
[Travis CI](https://app.travis-ci.com/github/xpack-dev-tools/{{gitHubProjectName}}/builds/).
{%- endif %}
...
git -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git pull
xpm run install -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
xpm run test-pre-release -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
```sh
xpm run trigger-workflow-publish-release -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
The result is a
[draft pre-release](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/releases/)
tagged like **v{customField('xpackVersion')}** (mind the dash in the middle!) and
named like **xPack {{packageConfig.longName}} v{customField('xpackVersion')}** (mind the dash),
...
```sh
xpm run generate-website-blog-post -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
```sh
xpm run update-package-binaries -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
<a href="https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}?activeTab=versions">npmjs.com</a>.
...
```sh
xpm run trigger-workflow-test-xpm -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
<CodeBlock language="sh"> {
`npm dist-tag add @xpack-dev-tools/{{packageConfig.shortName}}@${customField('xpackVersion')}.${customField('npmSubversion')} latest
`} </CodeBlock>

Check the result:

<CodeBlock language="sh"> {
`npm dist-tag ls @xpack-dev-tools/{{packageConfig.shortName}}
`} </CodeBlock>

In case the release proves problematic and needs to be unpublished:

<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/{{packageConfig.shortName}}@${customField('xpackVersion')}.${customField('npmSubversion')}
`} </CodeBlock>
...
in https://xpack-dev-tools.github.io/{{gitHubProjectName}}/blog/
- remember the post URL, since it must be used to update the release page
and the X/Twitter announcement
- switch to https://xpack-dev-tools.github.io/{{gitHubProjectName}}/docs/maintainer/#update-the-xpack-branch
...
- go to the [GitHub Releases](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/releases/) page
...
```sh
xpm run trigger-workflow-deep-clean -C ~/Work/xpack-dev-tools/{{gitHubProjectName}}.git/build-assets
```
...
- paste the release name like _**xPack {{packageConfig.longName}} v{customField('xpackVersion')} released**_
...
- GitHub [`xpack-dev-tools/{{gitHubProjectName}}`](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/) repo
  - latest xPack release
[![Github All Releases](https://img.shields.io/github/downloads/xpack-dev-tools/{{gitHubProjectName}}/latest/total.svg)](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/releases/)
  - all xPack releases [![Github All Releases](https://img.shields.io/github/downloads/xpack-dev-tools/{{gitHubProjectName}}/total.svg)](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/releases/){% if packageWebsiteConfig.customFields.showDeprecatedGnuMcuAnalytics == 'true' %}
  - previous GNU MCU Eclipse all releases [![Github All Releases](https://img.shields.io/github/downloads/gnu-mcu-eclipse/{{packageConfig.shortName}}/total.svg)](https://github.com/gnu-mcu-eclipse/{{packageConfig.shortName}}/releases/){% endif %}
...
- npmjs.com [`@xpack-dev-tools/{{packageConfig.shortName}}`](https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}/) xPack
  - latest release, per month
[![npm (scoped)](https://img.shields.io/npm/v/@xpack-dev-tools/{{packageConfig.shortName}}.svg)](https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}/)
[![npm](https://img.shields.io/npm/dm/@xpack-dev-tools/{{packageConfig.shortName}}.svg)](https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}/)
  - all releases [![npm](https://img.shields.io/npm/dt/@xpack-dev-tools/{{packageConfig.shortName}}.svg)](https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}/)
