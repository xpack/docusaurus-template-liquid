...
# Maintainer's Guide{% if packageWebsiteConfig.usePluralGuides == "true" %}s{% endif %}
...
# How to make new {{longXpackName}} releases
...
Determine the version (like <code>{{xpackVersion}}</code>)
and update the `build-assets/scripts/VERSION` file; the format
is <code>{{xpackVersion}}</code>.
...
and fix them; assign them to a milestone (like <code>{{xpackVersion}}</code>).
...
- add a new entry like _**\* v{{xpackVersion}} prepared**_
- commit with the message _**prepare v{{xpackVersion}}**_
...
to `"@{{githubProjectOrganization}}/xbb-helper":`
...
```sh
xpm run generate-workflows -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets
```
...
{% if isLinuxX64 %}xpm run trigger-workflow-test-docker-linux-x64 -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets{% endif %}
{% if isLinuxArm %}xpm run trigger-workflow-test-docker-linux-arm -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets{% endif %}
```
...
- in `CHANGELOG.md`, add the release date and a message
  like _**\* v{{xpackVersion}} released**_
...
tagged like **v{{xpackVersion}}** (mind the dash in the middle!) and
named like **{{longXpackName}} v{{xpackVersion}}** (mind the dash),
...
- commit with the message _**website: blog post release {{xpackVersion}} published**_
...
- switch to {{packageHomepage}}docs/maintainer/#update-the-xpack-branch
...
- paste the release name like _**{{longXpackName}} v{{xpackVersion}} released**_
...
[![Github All Releases](https://img.shields.io/github/downloads/{{githubProjectOrganization}}/{{githubProjectName}}/latest/total.svg)](https://github.com/{{githubProjectOrganization}}/{{githubProjectName}}/releases)
  - all xPack releases [![Github All Releases](https://img.shields.io/github/downloads/{{githubProjectOrganization}}/{{githubProjectName}}/total.svg)](https://github.com/{{githubProjectOrganization}}/{{githubProjectName}}/releases){% if packageWebsiteConfig.showDeprecatedGnuMcuAnalytics == 'true' %}
  - previous GNU MCU Eclipse all releases [![Github All Releases](https://img.shields.io/github/downloads/gnu-mcu-eclipse/arm-none-eabi-gcc/total.svg)](https://github.com/gnu-mcu-eclipse/arm-none-eabi-gcc/releases){% endif %}
