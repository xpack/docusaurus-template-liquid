...
- <code>xpack-{% if packageConfig.shortName %}{{packageConfig.shortName}}{% else %}{{packageName}}{% endif %}-{{releaseSemver}}-{{releaseSubversion}}-{props.platform}.tar.gz</code>
- <code>xpack-{% if packageConfig.shortName %}{{packageConfig.shortName}}{% else %}{{packageName}}{% endif %}-{{releaseSemver}}-{{releaseSubversion}}-{props.platform}.tar.gz.sha</code>

To update the build scripts and build the release binaries:

<CodeBlock language="sh"> {
`export XBB_ENVIRONMENT_MACOSX_DEPLOYMENT_TARGET='11.0' && \\
rm -f ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/package-lock.json && \\
git -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git pull && \\
xpm run install -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets && \\
\\
xpm run deep-clean --config ${props.platform}  -C ~/Work/{{githubProjectOrganization}}/{{githubProjectName}}.git/build-assets && \\
