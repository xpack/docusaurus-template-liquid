...
* identify the {% if isXpackBinary %}platform-specific archive for the {% endif %}desired version
...
* add <code>{{packageScopedName}}</code> to
  `package.json` as a
  {% if isXpackBinary %}development {% endif %}dependency; this associates a specific version of {{topConfig.preferredName}} with
