...
With the environment properly set, the command to install **{{packageScopedName}}** is:
...
`npm install --location=global {{packageScopedName}}@${customField('releaseVersion')} --verbose
`} </CodeBlock>
