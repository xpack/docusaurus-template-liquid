...
select the exact versions of the {% if isXpackBinary %}tools{% else %}libraries{% endif %} it requires.
...
required {% if isXpackBinary %}binary tools{% else %}libraries{% endif %} locally into each project.

Therefore, similarly to the way **npm** installs the JavaScript packages into
`node_modules`, **xpm** installs the {% if isXpackBinary %}binary tools{% else %}libraries{% endif %} into
