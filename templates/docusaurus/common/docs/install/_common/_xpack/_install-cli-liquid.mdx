...
Since the {% if isXpackBinary == "true" %}package provides executables{% elsif packageConfig.hasCli == "true" %}package includes command-line tools{% endif %}, it may also
...
<CodeBlock language="console"> {
`$ npx --package {{packageScopedName}} -- {{packageWebsiteConfig.programName}} --version
{{releaseVersion}}
`} </CodeBlock>
