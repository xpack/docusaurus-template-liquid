...
{%- if topConfig.permalinkName == 'xpm' %}
...
{% unless topConfig.hasCli %}dependency{% else %}development dependency{% endunless %} to any **npm**
...
Since the {% if isXpackBinary %}package provides executables{% elsif topConfig.hasCli %}package includes command-line tools{% endif %}, it may also
...
```txt
C:\>dir "%APPDATA%"\npm\{{websiteConfig.programName}}*
 Volume in drive C has no label.
 Volume Serial Number is B02D-925C

 Directory of C:\Users\ilg\AppData\Roaming\npm

29/07/2024  07:24               397 {{websiteConfig.programName}}
29/07/2024  07:24               329 {{websiteConfig.programName}}.cmd
29/07/2024  07:24               821 {{websiteConfig.programName}}.ps1
...
`C:\>{{websiteConfig.programName}} --version
{{releaseVersion}}
`} </CodeBlock>
...
<CodeBlock language="console"> {
`% which {{websiteConfig.programName}}
/Users/ilg/.nvm/versions/node/${props.nodeVersion}/bin/{{websiteConfig.programName}}
`} </CodeBlock>

To test if **{{websiteConfig.programName}}** starts:

<CodeBlock language="console"> {
`% {{websiteConfig.programName}} --version
{{releaseVersion}}
`} </CodeBlock>
...
<CodeBlock language="console"> {
`$ which {{websiteConfig.programName}}
/home/ilg/.nvm/versions/node/${props.nodeVersion}/bin/{{websiteConfig.programName}}
$ {{websiteConfig.programName}} --version
{{releaseVersion}}
`} </CodeBlock>
...
benefit from it by utilising the `npx` command, as a trampoline to start **{{websiteConfig.programName}}**:

<CodeBlock language="console"> {
`$ npx --package {{packageScopedName}} -- {{websiteConfig.programName}} --version
{{releaseVersion}}
`} </CodeBlock>
