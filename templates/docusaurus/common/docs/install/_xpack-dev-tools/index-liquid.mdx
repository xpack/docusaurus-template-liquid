...
title: Install Guide{% if packageWebsiteConfig.customFields.usePluralGuides == "true" %}s{% endif %}
...
keywords:
  ...
  - prerequisites
  - automated
  - install{% if packageWebsiteConfig.customFields.hasCustomInstall != "true" %}
  - uninstall
  - files{% endif %}
...
{% if packageWebsiteConfig.customFields.hasCustomInstall == "true" %}

import CustomInstall from './_custom-install.mdx'

{/* ------------------------------------------------------------------------ */}

# Install Guide{% if packageWebsiteConfig.customFields.usePluralGuides == "true" %}s{% endif %}
...
# How to install the xPack {{packageConfig.longName}}
...
[![GitHub Release Date](https://img.shields.io/github/release-date/xpack-dev-tools/{{gitHubProjectName}})](https://github.com/xpack-dev-tools/{{gitHubProjectName}}/releases/)
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2F{{packageConfig.shortName}}?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/{{gitHubProjectName}}/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2F{{packageConfig.shortName}}?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}/)

The **xPack {{packageConfig.longName}}** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install {{packageConfig.longName}}
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/{{packageConfig.shortName}}`}><code>@xpack-dev-tools/{{packageConfig.shortName}}</code></a> from
...
<code>xpacks/@xpack-dev-tools/{{packageConfig.shortName}}</code>.
...
**{{packageConfig.shortName}}** package into the project.

The command to install the **latest** available version of **{{packageConfig.shortName}}** is:

<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/{{packageConfig.shortName}}@latest --verbose
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/{{packageConfig.shortName}}@${customField('version')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of {{packageConfig.longName}}, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/{{packageConfig.shortName}}</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/{{packageConfig.shortName}}/.content/bin</code>;
* add <code>@xpack-dev-tools/{{packageConfig.shortName}}</code> to
  `package.json` as a
  development dependency; this associates a specific version of {{packageConfig.longName}} with
...
`  "xpack": {
    ...
    "dependencies": {},
    "devDependencies": {
      "@xpack-dev-tools/{{packageConfig.shortName}}": {
...
it is also possible to install {{packageConfig.longName}} only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/{{packageConfig.shortName}}@latest --global --verbose
`} </CodeBlock>
...
In order to access the {{packageConfig.longName}} binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/{{packageConfig.shortName}}/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/{{packageConfig.shortName}}/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/{{packageConfig.shortName}}/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/{{packageConfig.shortName}} --verbose
`} </CodeBlock>

To completely remove the package from the user global xPacks store:

<CodeBlock language="sh"> {
`xpm uninstall --global @xpack-dev-tools/{{packageConfig.shortName}} --verbose
`} </CodeBlock>
...
For all platforms, the **xPack {{packageConfig.longName}}** binaries are released as portable
archives that can be installed in any location.

The archives can be downloaded from
the <a href={`https://github.com/xpack-dev-tools/{{gitHubProjectName}}/releases/`}>GitHub Releases</a> pages.
...
The Windows versions of **xPack {{packageConfig.longName}}**
are packed as `.zip` files.
Download the latest version named like:

- <code>xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}-win32-x64.zip</code>
...
To manually install the <b>xPack {{packageConfig.longName}}</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\{{packageConfig.shortName}}</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\{{packageConfig.shortName}}\\xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}</code>.
...
The macOS versions of **xPack {{packageConfig.longName}}**
...
{% if isMacOSX64 %}- <code>xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}-darwin-x64.tar.gz</code>{% endif -%}
{% if isMacOsArm64 %}- <code>xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}-darwin-arm64.tar.gz</code>{% endif -%}
{% comment %}{% endcomment %}
...
The recommended location is
the <code>~/Library/xPacks/{{packageConfig.shortName}}</code> folder, for
example <code>/Users/ilg/Library/xPacks/{{packageConfig.shortName}}/xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/{{packageConfig.shortName}}
cd ~/Library/xPacks/{{packageConfig.shortName}}

tar xvf ~/Downloads/xpack-{{packageConfig.shortName}}-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-{{packageConfig.shortName}}-${customField('xpackVersion')}
`} </CodeBlock>
...
The GNU/Linux versions of **xPack {{packageConfig.longName}}**
are packed as `.tar.gz` archives.
Download the latest version named like:
...
{% if isLinuxX64 %}- <code>xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}-linux-x64.tar.gz</code>{% endif -%}
{% if isLinuxArm64 %}- <code>xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}-linux-arm64.tar.gz</code>{% endif -%}
{% if isLinuxArm %}- <code>xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}-linux-arm.tar.gz</code>{% endif -%}
{% comment %}{% endcomment %}
...
The recommended location is
the <code>~/.local/xPacks/{{packageConfig.shortName}}</code> folder, for
example <code>/home/ilg/.local/xPacks/{{packageConfig.shortName}}/xpack-{{packageConfig.shortName}}-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/{{packageConfig.shortName}}
cd ~/.local/xPacks/{{packageConfig.shortName}}

tar xvf ~/Downloads/xpack-{{packageConfig.shortName}}-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-{{packageConfig.shortName}}-${customField('xpackVersion')}
`} </CodeBlock>
