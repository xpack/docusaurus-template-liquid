...
# How to install the xPack {{appName}}
...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2F{{appLcName}}?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/{{appLcName}}/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/{{gitHubProjectName}}/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2F{{appLcName}}?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/{{appLcName}}/)

The **xPack {{appName}}** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install {{appName}}
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/{{appLcName}}`}><code>@xpack-dev-tools/{{appLcName}}</code></a> from
...
<code>xpacks/@xpack-dev-tools/{{appLcName}}</code>.
...
**{{appLcName}}** package into the project.

The command to install the **latest** available version of **{{appLcName}}** is:

<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/{{appLcName}}@latest --verbose
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/{{appLcName}}@${customField('version')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of {{appName}}, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/{{appLcName}}</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/{{appLcName}}/.content/bin</code>;
* add <code>@xpack-dev-tools/{{appLcName}}</code> to
  `package.json` as a
  development dependency; this associates a specific version of {{appName}} with
...
it is also possible to install {{appName}} only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/{{appLcName}}@latest --global --verbose
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/{{appLcName}}/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/{{appLcName}}/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/{{appLcName}}/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/{{appLcName}} --verbose
`} </CodeBlock>

To completely remove the package from the user global xPacks store:

<CodeBlock language="sh"> {
`xpm uninstall --global @xpack-dev-tools/{{appLcName}} --verbose
`} </CodeBlock>
...
For all platforms, the **xPack {{appName}}** binaries are released as portable
...
The Windows versions of **xPack {{appName}}**
are packed as `.zip` files.
Download the latest version named like:

- <code>xpack-{{appLcName}}-{customField('xpackVersion')}-win32-x64.zip</code>
...
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\{{appLcName}}</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\{{appLcName}}\\xpack-{{appLcName}}-{customField('xpackVersion')}</code>.
...
{% if isMacOSX64 %}- <code>xpack-{{appLcName}}-{customField('xpackVersion')}-darwin-x64.tar.gz</code>{% endif -%}
{% if isMacOsArm64 %}- <code>xpack-{{appLcName}}-{customField('xpackVersion')}-darwin-arm64.tar.gz</code>{% endif -%}
{% comment %}{% endcomment %}
...
The recommended location is
the <code>~/Library/xPacks/{{appLcName}}</code> folder, for
example <code>/Users/ilg/Library/xPacks/{{appLcName}}/xpack-{{appLcName}}-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/{{appLcName}}
cd ~/Library/xPacks/{{appLcName}}

tar xvf ~/Downloads/xpack-{{appLcName}}-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-{{appLcName}}-${customField('xpackVersion')}
`} </CodeBlock>
...
The GNU/Linux versions of **xPack {{appName}}**
are packed as `.tar.gz` archives.
Download the latest version named like:
...
{% if isLinuxX64 %}- <code>xpack-{{appLcName}}-{customField('xpackVersion')}-linux-x64.tar.gz</code>{% endif -%}
{% if isLinuxArm64 %}- <code>xpack-{{appLcName}}-{customField('xpackVersion')}-linux-arm64.tar.gz</code>{% endif -%}
{% if isLinuxArm %}- <code>xpack-{{appLcName}}-{customField('xpackVersion')}-linux-arm.tar.gz</code>{% endif -%}
{% comment %}{% endcomment %}
...
The recommended location is
the <code>~/.local/xPacks/{{appLcName}}</code> folder, for
example <code>/home/ilg/.local/xPacks/{{appLcName}}/xpack-{{appLcName}}-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/{{appLcName}}
cd ~/.local/xPacks/{{appLcName}}

tar xvf ~/Downloads/xpack-{{appLcName}}-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-{{appLcName}}-${customField('xpackVersion')}
`} </CodeBlock>
