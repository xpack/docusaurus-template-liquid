...
{%- unless websiteConfig.skipTests %}
...
> {% if isTypeScript %}ts-standard --fix src && {% endif %}standard --fix test
...
`} </CodeBlock>
...
{%- if useApiExtractor or websiteConfig.useApiDocumenter %}
{%- if isTypeScript %}
...
{%- if websiteConfig.has100coverage %}
...
The module is {% if websiteConfig.has100coverage %}tested with 100% coverage and is{% endif %}
...
Windows and macOS, utilising node {{ topConfig.githubActionsNodeVersions | join: ", " | replace_last: ", ", " and " }}.
