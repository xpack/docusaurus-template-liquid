...
like <code>{{releaseSemver}}-<b>{{xpackSubversion}}</b></code>. When
published as a npm package, the version gets
a fifth number,
like <code>{{xpackVersion}}.<b>{{npmSubversion}}</b></code>.

Since adherence of third party packages to SemVer is not guaranteed,
it is recommended to avoid referring to the {{longXpackName}} dependency via
a SemVer expressions
like <code>^{{xpackVersion}}</code> or <code>~{{xpackVersion}}</code>, and
